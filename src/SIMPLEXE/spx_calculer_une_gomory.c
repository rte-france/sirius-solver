...
/* Il est preferable de ne pas faire le calcul des gomory en hyper creux. De toutes fa�ons une gomory est
...
/* Remarque: les variables non bornees x = x+ - x- avec x+ >= 0 et x- >= 0 sont hors base a 0 
   c'est � dire x+ et x- sont nuls. Comme il y a une difference la variable n'intervient pas. */
for ( VarSpx = 0 ; VarSpx < Spx->NombreDeVariables ; VarSpx++ ) {
