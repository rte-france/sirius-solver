...
/* 
   ...
   - un pointeur � un objet de type PROBLEME_SIMPLEXE: il permet de renseigner les donnees
     du probleme � r�soudre
   - un pointeur � un objet de type PROBLEME_SPX. Lorsque la valeur de ce pointeur vaut NULL,
     SPX_Simplexe alloue un nouvel objet de type PROBLEME_SPX sur lequel il travaillera.
     Dans le cas contraire SPX_Simplexe travaillera sur l'objet de type PROBLEME_SPX pass�
     par l'appelant. 
     
   SPX_Simplexe renvoie un pointeur � un objet de type PROBLEME_SPX, ce pointeur
   correspond � l'objet sur lequel SPX_Simplexe vient de faire ses calculs.

   L'appelant peut de cette fa�on travailler sur plusieurs instances de problemes qu'il
   ...
   void * ProblemeSpx; <- Utiliser void * comme type de pointeur permet � l'appelant d'ignorer la
...
typedef struct {
  int  Contexte; /* Contexte dans lequel le simplexe est utilise. Cet argument peut prendre 3 valeurs:
                      BRANCH_AND_BOUND_OU_CUT: le simplexe est appel� dans un contexte de Branch And Bound 
		                               ou de Branch And Cut 
                      BRANCH_AND_BOUND_OU_CUT_NOEUD: le simplexe est appel� dans un contexte de Branch And Bound 
		                                     ou de Branch And Cut mais on ne reinitialise pas le probleme 
                      SIMPLEXE_SEUL: le simplexe est appel� hors d'un contexte de Branch and Bound ou de 
		                     Branch And Cut (dans ce cas, certaines sauvegardes particuli�res ne sont
				     pas faites) */
  ...
  int TypeDePricing;  /* Le pricing est l'�tape du calcul dans laquelle on choisit la variable sortante
			  dans l'algorithme dual (ou la variale entrante dans l'algorithme primal).  
			  Deux choix sont possibles:
			  * PRICING_DANTZIG: c'est la m�thode basique, elle est rapide mais dans certains
			                     cas conduit � faire beaucoup d'it�rations pour trouver l'optimum.
			  * PRICING_STEEPEST_EDGE: m�thode �labor�e (Forrest-Goldfarb), elle demande plus de
			    calculs mais permet de r�duite significativement le nombre d'it�rations. Il est
			    recommander de l'utiliser pour les probl�mes difficiles. */
  int FaireDuScaling; /* Vaut OUI_SPX ou NON_SPX. Si l'utilisateur positionne la valeur a OUI_SPX,
		 	  le simplexe fait un scaling du probleme d�s le d�but de la r�solution.
			  Le scaling a pour but d'am�liorer le conditionnement du probl�me. Il est
			  recommand� de l'utiliser lorsque les coefficients de la matrice des contraintes
			  sont tr�s diff�rents les un des autres (rapport > 100) */
  int StrategieAntiDegenerescence; /* Vaut AGRESSIF ou PEU_AGRESSIF.
                                       * AGRESSIF: le controle est fait � chaque iterations.
				       * PEU_AGRESSIF: le controle est fait moins souvent.
				       -> Choix recommand�: AGRESSIF
				    */				       
  ...
  double CoutMax; /* En entree: cette information n'est utilisee que si l'algorithme choisi est l'algorithme dual. 
		     ...
		     Il est donc possible de comparer ce cout � un Cout Max, seuil au dessus duquel on convient d'arreter les 
		     ...
		     probleme relax� menant a un cout superieur a ce cout sont a rejeter. Donc, si l'on se rend compte au cours de 
		     l'algorithme dual, que la resolution du probleme relaxe va mener a un cout trop grand il est inutile de 
                     ...
                     ATTENTION: comme l'algorithme dual peut etre utilis� en tant que solveur (c'est � dire 
