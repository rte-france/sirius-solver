...
# endif
	...
	Spx->CoutMax = Spx->spx_params->LINFINI_POUR_LE_COUT; /* Attention, ce n'est qu'une securite de plus, il vaut
	...
	for (i = 0; i < NbVar_E; i++) {
		...
		CorrectionDuale[NombreDeVariables] = Spx->spx_params->NOMBRE_MAX_DE_PERTURBATIONS_DE_COUT;
		...
		if (TypeVar_E[i] == VARIABLE_BORNEE_DES_DEUX_COTES) TypeDeVariable[NombreDeVariables] = BORNEE;
			...
			Xmin[NombreDeVariables] = -Spx->spx_params->LINFINI_POUR_X;
		...
		if (NombreDeTermes == 0) { /* Pas de nouvelle contrainte => fin de boucle */
		  /* printf("PAS NOUVELLE CONTRAINTE\n"); */
			if (TypeDeContrainte_E[Cnt_E] == '=') {
				if (fabs(S - B_E[Cnt_E]) > Spx->spx_params->SEUIL_DADMISSIBILITE) {
					Spx->YaUneSolution = NON_SPX; return;
				}
			}
			else if (TypeDeContrainte_E[Cnt_E] == '<') {
				if (S > B_E[Cnt_E] + Spx->spx_params->SEUIL_DADMISSIBILITE) {
					Spx->YaUneSolution = NON_SPX; return;
				}
			}
			else if (TypeDeContrainte_E[Cnt_E] == '>') {
	...
	for (i = 0; i < Spx->NombreDeVariables; i++) {
		...
		C[i] = -C[i];
		Xmin[i] = -Xmax[i];
		Xmax[i] = Spx->spx_params->LINFINI_POUR_X;
	}
...
FinVariablesBorneesSuperieurement:
	...
	for (i = 0; i < Spx->NombreDeVariables; i++) {
		ScaleX[i] = 1.;
		SeuilDeViolationDeBorne[i] = Spx->spx_params->SEUIL_DE_VIOLATION_DE_BORNE;
	}
	...
	for (i = 0; i < Spx->NombreDeVariables; i++) {
		...
		SeuilDAmissibiliteDuale2[i] = Spx->spx_params->SEUIL_ADMISSIBILITE_DUALE_2;
	}
	...
	if (Spx->LaBaseDeDepartEstFournie == NON_SPX) {
		SPX_DualConstruireUneCrashBase(Spx);
	}else {
	...
	for (i = NombreDeVariablesNatives; i < Spx->NombreDeVariables; i++) {
		ScaleX[i] = 1.;
		CorrectionDuale[i] = Spx->spx_params->NOMBRE_MAX_DE_PERTURBATIONS_DE_COUT;
	}
...
void SPX_MettreLaContrainteSousFormeStandard(PROBLEME_SPX * Spx, int Cnt)
{
	...
	if (Smin > (B[Cnt] + Spx->spx_params->SEUIL_DADMISSIBILITE)) {
		if (Spx->spx_params->VERBOSE_SPX) {
...
CreationDeLaVariableDEcart:
	...
	Xmax[Spx->NombreDeVariables] = Spx->spx_params->LINFINI_POUR_X;
	TypeDeVariable[Spx->NombreDeVariables] = BORNEE_INFERIEUREMENT;

	Seuil = Spx->spx_params->SEUIL_DE_VIOLATION_DE_BORNE_NON_NATIVE * ScaleB[Cnt];
	...
	Spx->SeuilDAmissibiliteDuale2[Spx->NombreDeVariables] = Spx->spx_params->SEUIL_ADMISSIBILITE_DUALE_2;
