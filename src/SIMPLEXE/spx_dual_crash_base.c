...
# define SEUIL_DADMISSIBILITE_TRIANGLE   ( 1.0 * Spx->spx_params->SEUIL_DADMISSIBILITE )
...
void SPX_DualConstruireUneCrashBase(PROBLEME_SPX * Spx)
{
	...
	//	printf("%6d %6d\n", i, (int)Spx->PositionDeLaVariable[i]);
	//}
	//for (Cnt = 0; Cnt < Spx->NombreDeContraintes; Cnt++)
	//	printf("Cnt %6d NbTerm %8d, %8d\n", Cnt, Cb->NbTerm[Cnt], Cb->PlusGrandNombreDeTermesParLigne);
	//for (Var = 0; Var <= Spx->NombreDeVariables; ++Var)
	//	printf("Var %6d PremLigne %6d\n", Var, Cb->PremLigne[Var]);
...
void SPX_DualCreerVariableDeBase(PROBLEME_SPX * Spx, int Cnt)
{
	...
	Seuil = Spx->spx_params->SEUIL_DE_VIOLATION_DE_BORNE_NON_NATIVE * Spx->ScaleB[Cnt];
