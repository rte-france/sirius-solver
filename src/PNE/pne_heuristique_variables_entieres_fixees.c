...
if ( OK == NON_PNE ) {
  ...
  Pne->NombreDEchecsSuccessifsHeuristiqueFixation++;
	if ( Pne->NombreDEchecsSuccessifsHeuristiqueFixation >= Pne->pne_params->NB_MAX_ECHECS_SUCCESSIFS_HEURISTIQUE ) {
    ...
    if ( Pne->NombreDeSolutionsHeuristiqueFixation <= 0 ) Pne->NombreDeReactivationsSansSuccesHeuristiqueFixation++;
		if ( Pne->NombreDeReactivationsSansSuccesHeuristiqueFixation > Pne->pne_params->NB_MAX_REACTIVATION_SANS_SUCCES ) {
