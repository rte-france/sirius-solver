...
if ( OK == NON_PNE ) {	
  Pne->NombreDEchecsSuccessifsHeuristiqueRINS++;
	if ( Pne->NombreDEchecsSuccessifsHeuristiqueRINS >= Pne->pne_params->NB_MAX_ECHECS_SUCCESSIFS_HEURISTIQUE && 0 ) {
    ...
    if ( Pne->NombreDeSolutionsHeuristiqueRINS <= 0 ) Pne->NombreDeReactivationsSansSuccesHeuristiqueRINS++;
		if ( Pne->NombreDeReactivationsSansSuccesHeuristiqueRINS > Pne->pne_params->NB_MAX_REACTIVATION_SANS_SUCCES ) {
