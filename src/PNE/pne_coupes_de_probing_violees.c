...
void PNE_DetectionDesCoupesDeProbingViolees( PROBLEME_PNE * Pne )
{
	...
	if ( Pne->pne_params->UTILISER_LE_GRAPHE_DE_CONFLITS == OUI_PNE && Pne->pne_params->PROBING_JUSTE_APRES_LE_PRESOLVE == OUI_PNE ) {
		return;
	}
	...
	Marge = Pne->pne_params->SEUIL_VIOLATION_COUPES_DE_PROBING;
