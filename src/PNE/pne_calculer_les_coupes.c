...
void PNE_CalculerLesCoupes(PROBLEME_PNE * Pne)
{
	...
	if ( Pne->pne_params->UTILISER_LE_GRAPHE_DE_CONFLITS == OUI_PNE ) {
		...
		if( Pne->pne_params->UTILISER_LES_COUPES_DE_PROBING == OUI_PNE ) {
	...
	*/
	...
	if (Bb->CalculerDesCoupesDeGomory == OUI_PNE && Bb->NoeudEnExamen->ProfondeurDuNoeud <= Plim) {
		...
		if (Pne->pne_params->CALCULER_COUPES_DE_GOMORY == OUI_PNE) {
	...
	*/
	...
	if (Pne->pne_params->CALCULER_COUPES_KNAPSACK_SIMPLE == OUI_PNE) {
		if (Pne->PremFrac >= 0) {
	...
	/* Marchand Wolsey */
	if( Pne->pne_params->CALCULER_MIR_MARCHAND_WOLSEY == OUI_PNE ) {
		if (Pne->PremFrac >= 0) {
