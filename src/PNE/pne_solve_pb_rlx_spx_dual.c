...
Probleme.AffichageDesTraces = Pne->pne_params->AffichageDesTraces;

if ( Pne->DureeDuPremierSimplexe <= 0. ) time( &HeureDeCalendrierDebut );

Pne->ProblemeSpxDuSolveur = SPX_Simplexe( &Probleme , Pne->ProblemeSpxDuSolveur , Pne->pne_params->spx_params );
