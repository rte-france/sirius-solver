...
for ( Cnt = 0 ; Cnt < Pne->NombreDeContraintesTrav ; Cnt++ ) {
  if ( ContrainteKnapsack[Cnt] == IMPOSSIBLE ) continue;
	if ( NbTermTrav[Cnt] > Pne->pne_params->NB_TERMES_FORCE_CALCUL_DE_K ) {
	  if ( fabs( VariableDuale[Cnt] ) < Pne->pne_params->SEUIL_VARIABLE_DUALE_POUR_CALCUL_DE_COUPE ) continue;
	}
	Inf = IMPOSSIBLE;
	Sup = IMPOSSIBLE;	
    ...
    /* Recherche couverture de sac a dos */		
		if ( NombreDeTermes >= Pne->pne_params->MIN_TERMES_POUR_KNAPSACK ) {
      ...
      if ( CouvertureTrouvee == OUI_PNE ) {
		...
		/* On essaie maintenant de remplacer les variables de cette contrainte par d'autres variables en combinant
			 les contraintes */
    if ( /*CouvertureTrouvee == NON_PNE &&*/ NbTermTrav[Cnt] >= Pne->pne_params->MIN_TERMES_POUR_KNAPSACK
		     && Pne->pne_params->KNAPSACK_SUR_COMBINAISONS_DE_CONTRAINTES == OUI_PNE ) {
