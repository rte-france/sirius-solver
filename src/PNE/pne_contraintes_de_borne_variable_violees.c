...
void PNE_DetectionDesContraintesDeBorneVariableViolees( PROBLEME_PNE * Pne )
{
	...
	if (Pne->pne_params->CONSTRUIRE_BORNES_VARIABLES == NON_PNE) {
		return;
	}
		
	if( Pne->pne_params->UTILISER_LE_GRAPHE_DE_CONFLITS == OUI_PNE && Pne->pne_params->PROBING_JUSTE_APRES_LE_PRESOLVE == NON_PNE ) {
		return;
	}
